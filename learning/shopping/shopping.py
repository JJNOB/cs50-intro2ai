...
def load_data(filename):
    ...
    following values, in order:
        ...
        - BounceRates, a flnt number
        - ExitRates, a floating point oating poinumber
    ...
    df = df.astype({"Administrative": int, "Administrative_Duration": float, "Informational": int,
                    "Informational_Duration": float, "ProductRelated": int, "ProductRelated_Duration": float,
                    "BounceRates": float, "ExitRates": float, "PageValues": float, "SpecialDay": float,
                    "OperatingSystems": int, "Browser": int, "Region": int,
                    "TrafficType": int, "Weekend": int, "Revenue": int})
